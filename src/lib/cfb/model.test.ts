...
import type { CfbTeamRating } from "./types";
...
describe("predictGame", () => {
  ...
  it("always sums win probabilities to exactly 1", () => {
    const cases: [CfbTeamRating, CfbTeamRating][] = [
  ...
  it("stays numerically stable (finite, in [0,1]) for extreme projected margins", () => {
    ...
    expect(extremeFavorite.homeWinProb).toBeCloseTo(1, 6); // an overwhelming favorite should saturate near 1, not NaN
  });
});
...
describe("spreadCoverProbability", () => {
  ...
  it("gives a favorite (negative home spread) a lower cover probability than a generous underdog spread, for the same projected margin", () => {
  ...
  it("reads as a coin flip when the entered spread exactly matches the projected margin", () => {
    ...
    expect(homeCoverProb).toBeCloseTo(0.5, 6); // 6 places: the erf approximation has ~1e-7-scale error, not exact
  });
...
describe("totalOverProbability", () => {
  it("sums over and under probability to exactly 1", () => {
    const { overProb, underProb } = totalOverProbability(54, 50);
  ...
  it("gives a higher over probability when the market total is below the projection, and vice versa", () => {
    const marketLow = totalOverProbability(54, 44);
    const marketHigh = totalOverProbability(54, 64);
  ...
  it("reads as a coin flip when the entered total exactly matches the projected total", () => {
    const { overProb } = totalOverProbability(54, 54);
    expect(overProb).toBeCloseTo(0.5, 6); // 6 places: the erf approximation has ~1e-7-scale error, not exact
  });
});
