 ...
 * and grader). NFL, tennis, soccer, and F1 register as thin market/results-lite
 * adapters: real `meta` and `ingest`, but an empty board until their paid-EV
 * lens lands — F1 is results-only (`meta.resultsOnly`), and NFL + tennis are
 * `meta.signalOnly` (priceable, but no results feed to grade them). Adding a
...
export const SPORTS: SportAdapter[] = [mlbAdapter, nflAdapter, ufcAdapter, tennisAdapter, soccerAdapter, f1Adapter];
