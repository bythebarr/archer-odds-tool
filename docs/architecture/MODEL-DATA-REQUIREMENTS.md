...
| Category | Status |
|---|---|
| Existing stored data | `Game`, `OddsSnapshot`, `CurrentOddsLine`, `GameOutcome` are already populated for NFL via `poll-odds-nfl`/`sync-results-nfl` crons (`docs/architecture/EDGE-BASELINE-AUDIT.md` §6). A separate, manual, experimental research writer also exists (`docs/architecture/NFL-RESEARCH.md`, `/nfl/research`) writing the Elo model's own win-probability/margin output to the generic `PredictionRun`/`ModelPrediction` tables — it stores no market line and does not change anything else in this row; CLV/spread/total-market evaluation for NFL is still fully answered (negatively) by the existing backtest below, not by this writer. |
